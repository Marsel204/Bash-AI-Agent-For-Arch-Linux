...
git clone https://github.com/Marsel204/Bash-AI-Agent-For-Arch-Linux.git
cd Bash-AI-Agent-For-Arch-Linux
...
- Copy the agent to `~/.local/share/arch-agent/`
- Install Python dependencies via `pip install --user`
- Create an `agent` command at `~/.local/bin/agent`
...
╭──────────────────────────────────────────────╮
│     🤖  Arch Linux AI System Agent           │
│  Local: driaforall/tiny-agent-a:0.5b         │
...
| Issue | Fix |
|-------|-----|
| `agent: command not found` | Make sure `~/.local/bin` is in your PATH (see Installation) |
...
| `ModuleNotFoundError` | Re-run `bash install.sh` to reinstall dependencies |
